...
// Return Type  : void
//
void controller_comp(const double x[4], double u_comp[6])
{
  double dv5[24];
  double dv[12];
  double dv1[12];
  ...
  // %% Unpacking
  // ydot = x(5:8);
  // ad = ydot(1); bd = ydot(2); gd = ydot(3); dd = ydot(4);
  //  I1 = par.inertia1; I2 = par.inertia2;
  // %% Jacobians
  // jacobian_JT1
  ...
  //  at2bar = Jt2_dot*ydot;
  //
  //  Jr1_dot = [0, -ad*sa,  ad*ca*cb-bd*sa*sb, 0;
  //             0,      0,             -bd*cb, 0;
  //             0, -ad*ca, -ad*sa*cb-bd*ca*sb, 0];
  //  ar1bar = Jr1_dot*ydot;
  //
  //  Jr2_dot = [0, -ad*sa, 0,  ad*ca*cb-bd*sa*sb;
  //             0,      0, 0,             -bd*cb;
  //             0, -ad*ca, 0, -ad*sa*cb-bd*ca*sb];
  //  ar2bar = Jr2_dot*ydot;
  // %% Forces
  //  M = Jt1'*m1*Jt1 + Jt2'*m2*Jt2 + Jr1'*I1*Jr1 + Jr2'*I2*Jr2;
  //  k = Jt1'*m1*at1bar + Jt2'*m2*at2bar + Jr1'*I1*ar1bar + Jr2'*I2*ar2bar +
  //  ...
  //      Jr1'*cross(w1, I1*w1) + Jr2'*cross(w2, I2*w2);
  // %% Equations of Motion
  //  yddot = M\(qe - k);
  //  x_dot = [ydot; yddot];
  // %% Control law
  ...
  dv[0] = (((d1 + param.d4[1] * t32_tmp) - param.d4[0] * t37_tmp) + d2) +
          d * t3_tmp;
  dv[1] = 0.0;
  d3 = param.d4[2] * t3_tmp;
  d4 = -param.d2[0] * t19_tmp + param.d2[1] * t20_tmp;
  d5 = param.d2[2] * t3_tmp * t5_tmp;
  dv[2] = (((d4 - param.d4[0] * t33_tmp) + param.d4[1] * t36_tmp) - d5) -
          d3 * t5_tmp;
  ...
  dv[4] = ((((d11 - d3) - d12) - d13) - param.d4[1] * b_t5_tmp * t6_tmp) -
          param.d4[0] * t6_tmp * b_t9_tmp;
  d3 = (-param.d2[2] * t2_tmp * t6_tmp + d6 * t9_tmp) + d7 * t10_tmp;
  dv[5] = ((d3 - d * t6_tmp) + d8 * t13_tmp) + d9 * t16_tmp;
  ...
  dv[8] = d7;
  dv[9] = -param.d4[1] * t33_tmp - param.d4[0] * t36_tmp;
  dv[10] = d9 * b_t5_tmp - d8 * b_t9_tmp;
  dv[11] = param.d4[0] * t32_tmp + param.d4[1] * t37_tmp;
  dv1[0] = (param.d1[1] * t18_tmp - param.d1[0] * t21_tmp) +
           param.d1[2] * t2_tmp * t3_tmp;
  dv1[4] = 0.0;
  dv1[8] = (-param.d1[0] * t19_tmp + param.d1[1] * t20_tmp) -
           param.d1[2] * t3_tmp * t5_tmp;
  d8 = param.d1[0] * t3_tmp;
  d9 = param.d1[1] * t3_tmp;
  dv1[1] = (d8 * t8_tmp - param.d1[2] * t5_tmp * t6_tmp) + d9 * t11_tmp;
  dv1[5] = (-param.d1[2] * t3_tmp - param.d1[1] * t4_tmp * t6_tmp) -
           param.d1[0] * t6_tmp * t7_tmp;
  dv1[9] = (-param.d1[2] * t2_tmp * t6_tmp + d8 * t10_tmp) + d9 * t9_tmp;
  dv1[2] = -param.d1[0] * t20_tmp - param.d1[1] * t19_tmp;
  dv1[6] = d8 * t4_tmp - d9 * t7_tmp;
  dv1[10] = param.d1[0] * t18_tmp + param.d1[1] * t21_tmp;
  dv1[3] = 0.0;
  dv1[7] = 0.0;
  dv1[11] = 0.0;
  t7_tmp = -param.mass1 * param.g;
  d8 = param.d3[2] * t2_tmp;
  dv2[0] = (((d1 + param.d3[1] * t32_tmp) - param.d3[0] * t37_tmp) + d2) +
           d8 * t3_tmp;
  dv2[1] = 0.0;
  d1 = param.d3[2] * t3_tmp;
  dv2[2] = (((d4 - param.d3[0] * t33_tmp) + param.d3[1] * t36_tmp) - d5) -
           d1 * t5_tmp;
  d2 = param.d3[1] * t3_tmp;
  d4 = param.d3[0] * t3_tmp;
  dv2[3] =
      ((d10 + d4 * b_t11_tmp) - param.d3[2] * t5_tmp * t6_tmp) + d2 * t17_tmp;
  dv2[4] = ((((d11 - d1) - d12) - d13) - param.d3[1] * b_t5_tmp * t6_tmp) -
           param.d3[0] * t6_tmp * b_t9_tmp;
  dv2[5] = ((d3 - d8 * t6_tmp) + d2 * t13_tmp) + d4 * t16_tmp;
  dv2[6] = d;
  dv2[7] = d6;
  dv2[8] = d7;
  dv2[9] = -param.d3[1] * t33_tmp - param.d3[0] * t36_tmp;
  dv2[10] = d4 * b_t5_tmp - d2 * b_t9_tmp;
  dv2[11] = param.d3[0] * t32_tmp + param.d3[1] * t37_tmp;
  t4_tmp = -param.mass2 * param.g;
  for (int i{0}; i < 4; i++) {
    dv4[i] = dv1[i + 4] * t7_tmp + dv2[3 * i + 1] * t4_tmp;
    dv3[i] = 0.0;
  }
  for (int i{0}; i < 3; i++) {
    int i1;
  ...
  dv5[18] = 0.0;
  dv5[22] = 0.0;
  dv5[15] = t3_tmp * t5_tmp;
  dv5[19] = -t6_tmp;
  dv5[23] = t2_tmp * t3_tmp;
  r = _mm_loadu_pd(&dv4[0]);
  r1 = _mm_loadu_pd(&dv3[0]);
  r2 = _mm_set1_pd(-1.0);
  _mm_storeu_pd(&dv4[0], _mm_mul_pd(_mm_add_pd(r, r1), r2));
  r = _mm_loadu_pd(&dv4[2]);
  r1 = _mm_loadu_pd(&dv3[2]);
  _mm_storeu_pd(&dv4[2], _mm_mul_pd(_mm_add_pd(r, r1), r2));
  coder::mldivide(dv5, dv4, u_comp);
